...
identifier `_`; B2 + B4 (1.20 → 1.21) skip `canonical_id` for function-local
`var`s (only package-level const/var get one) and drop proto's doubled
`proto:` prefix. Full per-bump rationale:
