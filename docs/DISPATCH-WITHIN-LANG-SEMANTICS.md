...
| W-C W7 (Sol cross-contract / storage / modifier) | W-C W6 완료 | 🔄 **IN PROGRESS — W7.1 ✅ LANDED 2026-05-17** (V0 low-level call dispatch: `runLowLevelCalls` walker in `low_level_call.go` matches `target.call/delegatecall/staticcall(...)` on bare-identifier receivers via `matchStateVarMethodCall` + property-name filter; `resolveLowLevelCallRef` looks up receiver type via W6 lookupReceiverType chain, byName[NodeInterface] preferred over byName[NodeContract], emits EdgeInvokes ConfAmbiguous with DispatchKind="low_level_call". `address(x)` cast / chain receivers drop in V0 per W7-D2. W7.2 storage location SubKind + W7.3 modifier composition pending). |
