...
| W-C W6 (Sol using For) | W-C W1 완료 | ✅ **LANDED 2026-05-12** (V0 binding + V1.0-V1.13 14-tier dispatch + V1.14-V1.21 family validations + V1.22-V1.24 callable kinds + V1.25-V1.27 lightweight guards + V1.28-V1.29 import alias + V1.30 block-shadow V0 + V2.0 line-range scope-aware + V2.1 interface receiver guard + **V2.2 multi-binding for same type** (bindings → multi-value, resolveBindingLib helper, 10 lookup site 통일); byte 정밀도 / module-import 추가 V2.3+; free-fn alias body / file-level using directive grammar-blocked) |
