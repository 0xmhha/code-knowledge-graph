...
| W-C W6 (Sol using For) | W-C W1 완료 | ✅ **LANDED 2026-05-12** (V0 binding + V1.0-V1.13 14-tier dispatch + V1.14 cross-file struct validation + V1.15 single-return local-var receiver + V1.16 multi-return tuple destructuring + V1.17 shadowing precedence fix + V1.18 cross-file tuple validation + V1.19 named return parameters → paramTypes + **V1.20 additional scope captures** (for-loop init regression guard + try/catch returns named param emit); block-scoped shadowing / module-import / catch_clause param V1.21+; free-fn / file-level grammar-blocked) |
