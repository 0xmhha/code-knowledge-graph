...
| W-C W6 (Sol using For) | W-C W1 완료 | ✅ **LANDED 2026-05-12** (V0 binding + V1.0-V1.13 14-tier dispatch + V1.14 cross-file struct validation + V1.15 single-return local-var receiver + V1.16 multi-return tuple destructuring (LHS explicit) + **V1.17 Solidity shadowing precedence fix** (lookupReceiverType: local-var → param → state-var 으로 의미 정합); cross-file tuple validation / block-scoped shadowing V1.18+; free-fn / file-level grammar-blocked) |
