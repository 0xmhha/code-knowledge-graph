...
| W-C W6 (Sol using For) | W-C W1 완료 | ✅ **LANDED 2026-05-12** (V0 binding + V1.0-V1.13 14-tier dispatch + V1.14 cross-file struct validation + V1.15 single-return local-var receiver + V1.16 multi-return tuple destructuring + V1.17 shadowing precedence fix + V1.18 cross-file tuple validation + V1.19 named return parameters → paramTypes + V1.20 try-returns named param + **V1.21 catch_clause named parameter** (catch_clause 분기 추가, V1.20 emitTryReturnsBinding helper 재사용); block-scoped shadowing / module-import V1.22+; free-fn / file-level grammar-blocked) |
