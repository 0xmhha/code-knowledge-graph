...
| W-C W7 (Sol cross-contract / storage / modifier) | W-C W6 완료 | 📋 **DESIGN DRAFT 2026-05-17** (`docs/design/solidity-cross-contract-storage-modifier.md`) — 3 sub-tracks: W7.1 low-level call (`target.call/delegatecall/staticcall` → EdgeInvokes AMBIGUOUS with DispatchKind="low_level_call"), W7.2 storage location SubKind (state-var visibility/mutability/location + parameter location), W7.3 modifier composition (Edge.Order field for multi-modifier order + EdgeOverrides for modifier override). 8 decisions locked. Sequential execution W7.1 → W7.2 → W7.3, total ~600 LOC across 3 commits. |
