...
| G6 | A3 incremental 경로 재활성화 — option D (pending refs persistence) 1차 시도 실패 후 revert. tiny synthetic fixture(2 files)에서는 PASS했지만 go-stable-code(2142 files)에서 두 가지 critical regression 노출: (1) changed_in (G6 temporal) edges dangling — incremental이 Commit 노드를 새 ID로 re-emit하는데 reload된 changed_in은 옛 ID 가리킴 → graph.Validate 실패. (2) ALL 92201 calls edges 손실 (cold 92201 → warm 0) — Pass 2 re-emission이 reloaded pending refs와 dirty parse 사이의 node ID 매핑을 처리 못 함. revert + 분석. 진짜 fix는: (a) temporal pass 결과를 incremental 경로에서 별도 처리 (cluster/score/temporal는 항상 full recompute 인 점을 incremental 진입 시점에 인지), (b) Pass 2 Resolve가 cached node IDs와 freshly-parsed node IDs 사이 namespace를 어떻게 배치하는지 spec-level 재검토 필요. C1 (Phase 2 reverse-reference index)와 묶어서 spec design 문서 먼저 작성 후 implement 권장. 현재는 partial-cache → cold fallback 상태 유지 (commit a684239 그대로). | ⏭️ G6 dedicated design + impl session 필요 |
...
| G9 | B1 Mutex detection 부분 누락 조사 — go-stable-code의 5개 named mutex만 검출됨 (`mu`, `mutex`, `batchMu`, `lock`, `wmu`, `errMu`, `l`)인데 781 acquires_lock 호출이 그것만 target하는 건 비현실적. corpus는 embedded 패턴 거의 안 씀 (`grep -E "^\s+sync\.Mutex$"` 5건만), 따라서 G9의 원래 가설(embedded 보강) 부적절. 진짜 누락 원인은 detection 일반 — `scanStructForMutex`가 file-top-level decl만 walk하고 nested struct/method-scoped struct는 누락하거나, 일부 import 별칭 (`syncpkg "sync"`)을 못 잡거나. dedicated investigation 필요. | ⏭️ G9 재조사 follow-up |
